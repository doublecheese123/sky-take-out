...
public class WorkspaceServiceImpl implements WorkspaceService {
    ...
    public BusinessDataVO getBusinessData(LocalDateTime begin, LocalDateTime end) {
        ...
        //查询总订单数
        Integer totalOrderCount = orderMapper.sumOrder(map);
        ...
        //有效订单数
        Integer validOrderCount = orderMapper.sumOrder(map);
    ...
    public OrderOverViewVO getOrderOverView() {
        ...
        //待接单
        Integer waitingOrders = orderMapper.sumOrder(map);
        waitingOrders = waitingOrders == null ? 0 : waitingOrders;

        //待派送
        map.put("status", Orders.CONFIRMED);
        Integer deliveredOrders = orderMapper.sumOrder(map);
        deliveredOrders = deliveredOrders == null ? 0 : deliveredOrders;

        //已完成
        map.put("status", Orders.COMPLETED);
        Integer completedOrders = orderMapper.sumOrder(map);
        completedOrders = completedOrders == null ? 0 : completedOrders;

        //已取消
        map.put("status", Orders.CANCELLED);
        Integer cancelledOrders = orderMapper.sumOrder(map);
        cancelledOrders = cancelledOrders == null ? 0 : cancelledOrders;

        //全部订单
        map.put("status", null);
        Integer allOrders = orderMapper.sumOrder(map);
